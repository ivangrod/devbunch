...
public final class FeedItemConverter {
	...
	public static FeedItem convertEntryToFeedItem(final String origin, final SyndEntry entry) {
		...
		builder.title(entry.getTitle()).uri(entry.getLink()).creator(entry.getAuthor()).origin(origin.toUpperCase());

		builder.generatedId(md5FromString(entry.getLink()));

		builder.publicationTimestamp(Optional.ofNullable(entry.getPublishedDate()).orElse(entry.getUpdatedDate()));
		...
		if (!CollectionUtils.isEmpty(entry.getCategories())) {
			Set<String> topicNameCollection = entry.getCategories().stream().map(syndCategory -> syndCategory.getName())
